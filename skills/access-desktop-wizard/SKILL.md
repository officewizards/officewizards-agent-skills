...
Use STA PowerShell for COM automation. Attach to an existing Access instance when possible, use explicit database/object references, prefer transactions for data changes when available, restore application state in `finally`, and return concise structured output with database path, affected objects, row counts/schema summary, and warnings.

## Verification

Finish Access work by reporting what changed and how it was verified. For destructive or schema work, include the verification query or object inspection used.
